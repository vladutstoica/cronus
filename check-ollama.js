...
async function checkOllama() {
  ...
  try {
    ...
    console.log(`   🏷️  Categorization Enabled: ${categorizationEnabled ? '✅ YES' : '❌ NO'}`)
    console.log(`   🤖 Ollama Model: ${ollamaModel}`)

    if (!aiEnabled) {
      console.log('   ⚠️  AI is DISABLED. Enable it in Settings or run:')
      console.log('      await window.electron.ipcRenderer.invoke("local:set-setting", "ai_enabled", "true")')
    }

    if (!categorizationEnabled) {
      console.log('   ⚠️  Categorization is DISABLED. Enable it with:')
      console.log('      await window.electron.ipcRenderer.invoke("local:set-setting", "categorization_enabled", "true")')
    }
  } catch (error) {
  ...
  try {
    ...
    if (exactMatch) {
      ...
      console.log(`   ⚠️  Configured model "${configuredModel}" not found exactly`)
      console.log(`   💡 Found similar model: "${partialMatch}"`)
      console.log(`   💡 Update setting to match:`)
      console.log(`      await window.electron.ipcRenderer.invoke("local:set-setting", "ollama_model", "${partialMatch}")`)
    } else {
  ...
  try {
    const events = await window.electron.ipcRenderer.invoke('local:get-events', 20, 0)
    ...
    console.log(`   ✅ Categorized: ${categorizedEvents.length}/${events.length}`)
    console.log(`   🤖 With AI reasoning: ${withReasoning.length}/${events.length}`)
    console.log(`   📝 With AI summary: ${withAISummary.length}/${events.length}`)
    ...
    if (withAISummary.length > 0) {
      ...
      console.log('      This means rule-based categorization is working, but AI may not be used yet')
    } else {
  ...
  if (aiEnabled) {
    console.log('💡 Summary: AI is enabled. New events should be categorized using Ollama.')
    console.log('💡 Monitor main process logs (terminal) to see Ollama categorization in action.')
  } else {
    console.log('⚠️  Summary: AI is DISABLED. Enable it in Settings to use Ollama.')
  }
}
