...
async function runDiagnostics() {
  ...
  try {
    ...
    console.log(`   ✅ Permissions: ${permissionInfo.missingAccessibilityPermissions ? '❌ DENIED' : '✅ GRANTED'}`)
  ...
  try {
    const events = await window.electron.ipcRenderer.invoke('local:get-events', 10, 0)
    console.log(`   📦 Total events in database (last 10): ${events.length}`)
    if (events.length > 0) {
      const latestEvent = events[0]
      console.log(`   📅 Latest event timestamp: ${new Date(latestEvent.timestamp).toLocaleString()}`)
      ...
      console.log(`   🏷️  Category: ${latestEvent.categoryId || 'Uncategorized'}`)
    } else {
      console.log('   ⚠️  No events found yet. Keep the app running and switch between applications.')
    }
  } catch (error) {
  ...
  try {
    const categories = await window.electron.ipcRenderer.invoke('local:get-categories')
    ...
    if (todayEvents.length > 0) {
      ...
      Object.entries(byApp)
        ...
        .forEach(([app, data]) => {
          ...
          console.log(`      - ${app}: ${data.count} events, ${minutes} minutes`)
        })
    }
  } catch (error) {
  ...
  try {
    const settings = await window.electron.ipcRenderer.invoke('local:get-settings')
    console.log(`   ⚙️  Tracking enabled: ${settings.tracking_enabled !== false ? '✅' : '❌'}`)
    console.log(`   🤖 Categorization enabled: ${settings.categorization_enabled !== false ? '✅' : '❌'}`)
    console.log(`   📸 Screenshots enabled: ${settings.screenshots_enabled ? '✅' : '❌'}`)
  ...
  console.log('💡 If you see events in the database, check the calendar view to see them displayed.')
  console.log('💡 If no events yet, keep the app running and switch between applications.')
}
