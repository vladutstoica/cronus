...
export const DistractionSoundSettings = () => {
  ...
  const [showDistractionNotifications, setShowDistractionNotifications] =
    useState(true);
  const [distractionNotificationInterval, setDistractionNotificationInterval] =
    useState(60); // in seconds
    ...
    try {
      ...
      if (userData && userData.electron_app_settings) {
        ...
        setShowDistractionNotifications(
          settings.showDistractionNotifications ?? true,
        );
        setDistractionNotificationInterval(
          settings.distractionNotificationInterval ?? 60,
        );
      }
    } catch (error) {
  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader>
        ...
        <CardDescription>
          Get alerts when you&apos;re using unproductive apps to help you stay
          focused.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="flex items-center justify-between space-x-4">
          <div className="flex items-center space-x-2">
            ...
            />
            <Label
              htmlFor="show-notifications-switch"
          ...
          >
            <SelectTrigger
              id="notification-interval-select"
              className="w-[180px]"
            >
