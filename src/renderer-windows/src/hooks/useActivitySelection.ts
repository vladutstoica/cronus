import {
  ActivityItem,
  ProcessedCategory,
} from "@renderer/lib/activityProcessing";
...
const useActivitySelection = (
  ...
  const [lastSelectedActivityKey, setLastSelectedActivityKey] = useState<
    string | null
  >(null);
  ...
  const allVisibleActivities = useMemo(() => {
    ...
    processedData.forEach((category) => {
      ...
      const visibleActivities = allCategoryActivities.filter(
        (act) => act.durationMs >= oneMinuteMs,
      );
      const hiddenActivities = allCategoryActivities.filter(
        (act) => act.durationMs < oneMinuteMs,
      );
  ...
  const handleSelectActivity = (
    ...
    if (shiftKey && lastSelectedActivityKey) {
      const allVisibleActivityKeys = allVisibleActivities.map(
        (a) => `${a.identifier}-${a.name}`,
      );
