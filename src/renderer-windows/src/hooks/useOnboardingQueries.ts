...
export function useOnboardingQueries() {
  ...
  const [existingReferralSource, setExistingReferralSource] =
    useState<string>("");
  ...
  useEffect(() => {
    console.log(
      "🚪 Onboarding modal mounted. Enabling permission requests for onboarding.",
    );
  ...
  const hasExistingGoals =
    typeof userProjectsAndGoals === "string" &&
    userProjectsAndGoals.trim().length > 0;
  const hasExistingReferral =
    typeof existingReferralSource === "string" &&
    existingReferralSource.trim().length > 0;

  const isLoading =
    isLoadingGoals || isLoadingHasCategories || isLoadingReferral;
