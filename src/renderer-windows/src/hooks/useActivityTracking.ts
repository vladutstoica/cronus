...
export function useActivityTracking({
  ...
  const [activeWindow, setActiveWindow] = useState<ActiveWindowDetails | null>(
    null,
  );
  const [isRecategorizeDialogOpen, setIsRecategorizeDialogOpen] =
    useState(false);
  const [recategorizeTarget, setRecategorizeTarget] =
    useState<ActivityToRecategorize | null>(null);
  ...
  useEffect(() => {
    const handleRecategorizeRequestFromIPC = (
      receivedData: ActivityToRecategorize,
    ): void => {
      ...
      if (
        ...
        console.warn(
          "App.tsx: IPC recategorize request failed. Data received:",
          receivedData,
        );
      }
    };
    ...
    if (window.api && window.api.onDisplayRecategorizePage) {
      const cleanup = window.api.onDisplayRecategorizePage(
        handleRecategorizeRequestFromIPC,
      );
      return cleanup;
    } else {
      console.warn(
        "App.tsx: window.api.onDisplayRecategorizePage not available for IPC.",
      );
      return () => {};
    }
