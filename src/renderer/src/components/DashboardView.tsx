...
const convertCalendarEventToBlock = (
  event: CalendarEvent,
...
export function DashboardView({
  ...
  const [googleCalendarProcessedEvents, setGoogleCalendarProcessedEvents] =
    useState<ProcessedEventBlock[] | null>(null); // Google Calendar events (empty array since no calendar integration)
  ...
  useEffect(() => {
    const calculateDateRange = (): void => {
      if (viewMode === "day") {
        ...
        startOfFourWeeks.setDate(
          selectedDate.getDate() - selectedDate.getDay() - 3 * 7 + 1,
        );
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      console.log(
        "📝 Setting events in service:",
        eventsWithParsedDates.length,
      );
  ...
  const activityWidgetProcessedEvents = useMemo(() => {
    ...
    if (viewMode === "week" && selectedDay) {
      return trackedProcessedEvents.filter(
        (block) =>
          block.startTime.toDateString() === selectedDay.toDateString(),
      );
    }
