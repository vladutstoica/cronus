...
export const getDisplayWindowInfo = (
  ...
  if (!dw)
    return {
      ownerName: "No active application",
      title: "",
      url: undefined,
      isApp: true,
    };
...
export const getStatusText = (
  ...
  if (isLoadingCategory || isLoadingUserCategories)
  ...
  const fullCategoryDetails = categoryDetails as Category;
  if (fullCategoryDetails.isProductive === true)
    return `${fullCategoryDetails.name}: Productive`;
  if (fullCategoryDetails.isProductive === false)
    return `${fullCategoryDetails.name}: Unproductive`;
