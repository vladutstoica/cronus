...
export class LMStudioProvider implements AIProvider {
  ...
  constructor(customBaseUrl?: string) {
    this.baseUrl = customBaseUrl || getSetting('lmstudio_base_url') || 'http://localhost:1234/v1';
  ...
  async generateChatCompletion(
    ...
    try {
      const model = getSetting('lmstudio_model') || (await this.getDefaultModel());
  ...
  pullModel?(modelName: string): Promise<boolean> {
    console.warn('LM Studio does not support pulling models via API. Please download models manually.');
